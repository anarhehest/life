...
void updateGrid(int grid[GRID_HEIGHT][GRID_WIDTH]) {
    int tmpGrid[GRID_HEIGHT][GRID_WIDTH];
    memset(tmpGrid, 0, sizeof(int) * GRID_HEIGHT * GRID_WIDTH);
    ...
    for (int y = 0; y < GRID_HEIGHT; y++) {
        for (int x = 0; x < GRID_WIDTH; x++) {
            ...
            if (grid[y][x] == 1) {
                tmpGrid[y][x] = (aliveNeighbors == 2 || aliveNeighbors == 3) ? 1 : 0;
            } else {
                tmpGrid[y][x] = (aliveNeighbors == 3) ? 1 : 0;
            }
        }
    }
